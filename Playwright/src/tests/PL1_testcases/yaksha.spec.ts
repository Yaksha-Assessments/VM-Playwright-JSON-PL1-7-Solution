 ...
 */
  ...
  await verifyFunction(page, compnameInput);
 ...
 */
  ...
  test("TS-2 Verify New 'Qualification' added could be edited from the record of user ", async ({ page }) => {
   ...
   await myinfoPage.editQualification(editInput);
    await verifyeditQualification(page, editInput);
 ...
 */
  ...
  test("TS-3 Verify the qualification could be deleted", async ({
    ...
    await  myinfoPage.deleteQualification(delUser,jobTitle)
     await verifyDeleteFunction(page, delUser);
  });
 ...
 */
  ...
  await verifyDeleteMultiple(page);
});
 ...
 */
  ...
  test("TS-5 Verify New 'Education' could be added to the record of user", async ({
   ...
   await verifyAddeducation(page, GPA);
  ...
  test("TS-6 Verify New 'Education' added could be edited from the record of user", async ({
    ...
    const GPA = generateUniqueGPA();
     await myinfoPage.editEducation(GPA);
      await verifyEditeducation(page, GPA);
 ...
 **/
  ...
  test("TS-7 Verify New 'Language' could be added to the record of user", async ({ page }) => {
    const comment = generateUniqueComment();
     ...
     await verifyAddlanguage(page, comment);
  });
 ...
 */
  ...
  test("TS-8 Verify New 'Language' could be delegted to the record of user", async ({ page }) => {
    const editComment =generateUniqueComment();
     ...
     await verifyDeletelanguage(page, editComment);
 ...
 * 2. Navigate to the Skills section and add a new skill entry.
 * 3. Retrieve the list of existing skills.
 * 4. Assert that the newly added skill is present in the list.
 */
  ...
  test("TS-9 Verify the 'Skills' could be added to the record", async ({
    ...
    await verifyAddSkills(page, year);
 ...
 */
  ...
  await verifyeditSkills(page, editYear);
});
...
async function verifyFunction(page: Page, compnameInput: string) {
...
async function verifyeditQualification(page: Page, editInput: string) {
...
async function verifyDeleteFunction(page: Page, delUser: string) {
...
// verify function for ts-4
async function verifyDeleteMultiple(page: Page) {
...
async function verifyAddeducation(page: Page, GPA: string) {
   const GPAList = await  page.locator("//div[@class='oxd-table-row oxd-table-row--with-border']/div[4]").allInnerTexts();
...
async function verifyEditeducation(page: Page, GPA: string) {
  const editGPAList = await page.locator("//div[@class='oxd-table-row oxd-table-row--with-border']/div[4]").allInnerTexts();
    expect(editGPAList && editGPAList.length > 0).toBeTruthy();

    expect(editGPAList).toContain(GPA);
}

// verify function for ts-7
async function verifyAddlanguage(page: Page, comment: string) {
...
async function verifyDeletelanguage(page: Page, editComment: string) {
    const table = page.locator("//div[@class='orangehrm-container']");
...
async function verifyAddSkills(page: Page, year: number) {
  const table = await page.locator("//div[@class='orangehrm-container']");
    const yearList =await  table.nth(2).locator("//div[@role = 'row'][1]/div[3]/div").allInnerTexts();
    expect(yearList && yearList.length > 0).toBeTruthy();
    expect(yearList).toContain(year.toString());
    await  table.nth(2).locator("//i[contains(@class,'bi-trash')]").first().click();
...
async function verifyeditSkills(page: Page, editYear: number) {
   const table =  page.locator("//div[@class='orangehrm-container']");
